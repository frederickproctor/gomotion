...
foreach row {1 2 3 4} {
    ...
    set number[expr $row + 3] [label $d.row$row.posr -textvariable pos[expr $row + 3] -width 10 -anchor e -font {Helvetica 12 bold}]
    label $d.row$row.namer -textvariable name[expr $row + 3] -width 2 -relief ridge -justify center
    radiobutton $d.row$row.rbr -variable whichselect -value [expr $row + 3] -command do_whichselect
...
proc popup_setup {w} {
	...
	label $w.row$row.namer -text [expr $row + 3] -width 2 -relief ridge -justify center
	radiobutton $w.row$row.rbr -variable setupselect -value [expr $row + 3] -command "update_setup $w"
