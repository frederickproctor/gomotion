...
//
// $Log: s626mod.c,v $
// Revision 1.2  2006/12/08 14:00:56  proctor
// Tested 2 boards
//
// Revision 1.1  2006/05/18 19:03:57  proctor
// Initial checkin
//
// 
//////////////////////////////////////////////////////////////////////////////////////////
...
typedef struct MOD_OBJ {						// ------------ MODULE-LEVEL BOARD OBJECT ------------
  ...
  void				*IntCallback; //   Address of application interrupt callback function.
  ...
  unsigned long		( *CounterReadLatch )		( HBD, unsigned short );					// for S626_CounterReadLatch().
  ...
  void				( *CounterPreload )			( HBD, unsigned short, unsigned long );		// for S626_CounterPreload().
  ...
  unsigned long		( *GetAddress )				( HBD );									// for S626_GetAddress().
  unsigned long		( *GetErrors )				( HBD );									// for S626_GetErrors().
...
static PVOID IntWait( PVOID pobj )
{
  ...
  for ( ; ; )
	...
	break;
      ...
      if ( !m->CallbackArg )
	( (FPTR_ISR)m->IntCallback )();										// Callback expects no arguments.
      else
	( ( void (*)(void *) )m->IntCallback )( (void *)m->CallbackArg );	// Callback is passed a pointer.
    }
}
...
static void LockBoard( struct MOD_OBJ *m, SX_PCI_SLOT *Loc, BOOLEAN MatchSubIDs, BYTE IsBoardRevA )
{
  ...
  if ( ioctl( m->hSX, S626_IOC_GetCardInfo, &PciCardInfo ) )
    {
      ...
      printf("LockBoard: error 1 registering board %d\n", (int) m->Board);
      return;
    }
  ...
  if (ioctl (m->hSX, S626_IOC_Register, &m->CardReg))
    {
      S626CORE_SetErrors( m->Board, ERR_CARDREG );	//ErrFlags |= ERR_CARDREG;
      printf("LockBoard: error 2 registering board %d\n", (int) m->Board);
      return;
    }
  ...
  if ( !m->CardReg.hCard )
    {
      ...
      printf("LockBoard: error 3 registering board %d\n", (int) m->Board);
      return;
    }
  ...
  for ( c = 0; c < PciScan.dwCards; c++ )
    {
      ...
      if ( m->CardReg.hCard ) {
	printf("RegisterAny: registered board %d\n", (int) c);
	return;
      }
    }
  
  printf("RegisterAny: no board registered\n");
...
static void Constructor( HBD bd, DWORD PhysLoc, VOID *callback_fn, DWORD Priority, DWORD callback_arg )
{
  struct MOD_OBJ		*m = &mo[bd];
  ...
  switch ( PhysLoc )
    ...
    case 0x00000000:
      ...
      if ( !m->CardReg.hCard ) RegisterAny( m, SUBID_626, REV_B );
      if ( m->CardReg.hCard ) {
	printf("registered REV B board %d\n", bd);
	break;
      }
      printf("can't register board %d as REV B\n", bd);
      ...
      RegisterAny( m, SUBID_ANY, REV_A );
      if ( m->CardReg.hCard ) {
	printf("registered REV A board %d\n", bd);
	break;
      }

      printf("can't register board %d as REV A either\n", bd);
      break;
...
// Counter functions:
unsigned long	S626_CounterReadLatch		(HBD hbd, unsigned short ch)						{ return BD.CounterReadLatch(hbd, ch); }
...
void			S626_CounterPreload			(HBD hbd, unsigned short ch, unsigned long val)		{ BD.CounterPreload(hbd, ch, val); }
void			S626_CounterIntSourceSet	(HBD hbd, unsigned short ch, unsigned short src)	{ BD.CounterIntSourceSet(hbd, ch, src); }

// Status and control functions:
unsigned long	S626_GetAddress				(HBD hbd)											{ return BD.GetAddress(hbd); }
unsigned long	S626_GetErrors				(HBD hbd)											{ return BD.GetErrors(hbd); }
...
void S626_OpenBoard( HBD hbd, unsigned long PhysLoc, FPTR_ISR Callback, unsigned long Priority )
{
  // Abort if a valid board handle is not specified.
  if ( hbd >= MAX_BOARDS )
    return;
