...
/*
  The pci_dev structure lost the slot_name member a while ago, I think
  it was dropped after 2.4 AFAIK. You should use the newer
  pci_name()...  You could try replacing all references to
  pci_dev->slot_name with pci_name(pci_dev) in the source,
  that should fix it.
*/
...
S626_init_one (struct pci_dev *pdev, const struct pci_device_id *ent)
{
  ...
  int i;
#if LINUX_VERSION_CODE < KERNEL_VERSION(2,6,0)
  char print_name[sizeof (pdev->slot_name)];
#else
  ...
  if (pci_enable_device (pdev))
	...
	board[S626_cards_found].S626_dma[i] =
      ...
      printk("S626: Detected S626 Card - Address=0x%x io_size=0x%x irq=%d Slot=%s\n", io_addr, io_size,
	   pdev->irq, pdev->slot_name);		//print_name);
#else
      printk("S626: Detected S626 Card - Address=0x%x io_size=0x%x irq=%d Slot=%s\n", io_addr, io_size,
...
S626_remove_one (struct pci_dev *pdev)
{
  ...
  for (i = 0; i < MAX_626_BOARDS; i++)
	...
	DBG ("request to remove board in slot %s current board slot %s %d\n",
	     pdev->slot_name, board[i].S626_dev->slot_name, i);
	if (strcmp (pdev->slot_name, board[i].S626_dev->slot_name) == 0)
	  {
	    DBG ("MATCH!!!\n");
	    remove = i;
	    break;
