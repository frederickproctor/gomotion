...
//																							//
// Copyright : (C) 2003 - 2004 Sensoray														//
//////////////////////////////////////////////////////////////////////////////////////////////
...
typedef struct CORE_OBJ {				// 626 BOARD'S CORE OBJECT ------------------------
	...
	U32				MyAddress;			//     PCI bus & slot number of card.
	U32				MyBase;				//     Board base memory address.
	...
	U32				*pDacWBuf;			//     Pointer to logical adrs of DMA buffer used to hold DAC data.
	U16				Dacpol;				//     Image of DAC polarity register.
	S16				DacSetpoint[4];		//     Images of DAC setpoints.
	U8				TrimSetpoint[12];	//     Images of TrimDAC setpoints.
	U16				DOutImage[2];		//     Images of DIO output registers.
...
static void		DummyErrHandler	( U32 ErrMask );

static U32		GetAddress		( struct CORE_OBJ *c );
...
#define WR7146( REGADRS, VAL )					S626MOD_WriteMem( (U32 *)( c->MyBase + (REGADRS) ), (VAL) )
#define RR7146( REGADRS )						S626MOD_ReadMem ( (U32 *)( c->MyBase + (REGADRS) )        )
...
UINT S626CORE_CoreOpen( HBD bd, UINT num_boards )
{
	...
	c->MyAddress	= 0;				// Reset board physical address.
...
void S626CORE_SetBase( HBD bd, U32 base )
{
	struct CORE_OBJ	*c;
...
static void WaitDoneADC( struct CORE_OBJ *c, U16 *pdata )
{
	...
	// Init ptr to DMA buffer that holds new ADC data.  We skip the first U16 in the buffer
...
static void ResetADC( struct CORE_OBJ *c, U8 *ppl )
{
	...
	pRPS = (U32 *)c->RPSBuf.LogicalBase;

	// Construct RPS program in RPSBuf DMA buffer ----------------------------------------
	
	// Wait for Start trigger from ReadADC().
	S626MOD_WriteMem( pRPS++, RPS_PAUSE | RPS_SIGADC );
	...
	S626MOD_WriteMem( pRPS++, RPS_LDREG | (P_DEBICMD >> 2) );		// Write DEBI Write command and address to shadow RAM.
	S626MOD_WriteMem( pRPS++, DEBI_CMD_WRWORD | LP_GSEL );
	S626MOD_WriteMem( pRPS++, RPS_LDREG | (P_DEBIAD >> 2) );			// Write DEBI immediate data to shadow RAM:
	S626MOD_WriteMem( pRPS++, GSEL_BIPOLAR5V );						//   arbitrary immediate data value.
	S626MOD_WriteMem( pRPS++, RPS_CLRSIGNAL | RPS_DEBI );			// Reset "shadow RAM uploaded" flag.
	S626MOD_WriteMem( pRPS++, RPS_UPLOAD    | RPS_DEBI );			// Invoke shadow RAM upload.
	S626MOD_WriteMem( pRPS++, RPS_PAUSE     | RPS_DEBI );			// Wait for shadow upload to finish.
	...
	for ( c->AdcItems = 0; c->AdcItems < 16; c->AdcItems++ )
	{
		...
		S626MOD_WriteMem( pRPS++, RPS_LDREG | (P_DEBICMD >> 2) );	// Write DEBI command and address to shadow RAM.
		S626MOD_WriteMem( pRPS++, DEBI_CMD_WRWORD | LP_GSEL );
		S626MOD_WriteMem( pRPS++, RPS_LDREG | (P_DEBIAD >> 2) );		// Write DEBI immediate data to shadow RAM:
		S626MOD_WriteMem( pRPS++, LocalPPL );
		S626MOD_WriteMem( pRPS++, RPS_CLRSIGNAL | RPS_DEBI );		// Reset "shadow RAM uploaded" flag.
		S626MOD_WriteMem( pRPS++, RPS_UPLOAD    | RPS_DEBI );		// Invoke shadow RAM upload.
		S626MOD_WriteMem( pRPS++, RPS_PAUSE     | RPS_DEBI );		// Wait for shadow upload to finish.

		// Select ADC analog input channel.
		S626MOD_WriteMem( pRPS++, RPS_LDREG | (P_DEBICMD >> 2) );	// Write DEBI command and address to shadow.
		S626MOD_WriteMem( pRPS++, DEBI_CMD_WRWORD | LP_ISEL );
		S626MOD_WriteMem( pRPS++, RPS_LDREG | (P_DEBIAD >> 2) );		// Write DEBI immediate data to shadow.
		S626MOD_WriteMem( pRPS++, LocalPPL );
		S626MOD_WriteMem( pRPS++, RPS_CLRSIGNAL | RPS_DEBI );		// Reset "shadow RAM uploaded" flag.
		S626MOD_WriteMem( pRPS++, RPS_UPLOAD    | RPS_DEBI );		// Invoke shadow RAM upload.
		S626MOD_WriteMem( pRPS++, RPS_PAUSE     | RPS_DEBI );		// Wait for shadow upload to finish.
		...
		JmpAdrs = (U32)c->RPSBuf.PhysicalBase + (U32)pRPS - (U32)c->RPSBuf.LogicalBase;
		for ( i = 0; i < ( 10 * RPSCLK_PER_US / 2); i++ )
		{
			JmpAdrs += 8;										// Repeat to implement time delay:
			S626MOD_WriteMem( pRPS++, RPS_JUMP );					//   Jump to next RPS instruction.
			S626MOD_WriteMem( pRPS++, JmpAdrs );
		}
		
		// Start ADC by pulsing GPIO1.
		S626MOD_WriteMem( pRPS++, RPS_LDREG | (P_GPIO >> 2) );		// Begin ADC Start pulse.
		S626MOD_WriteMem( pRPS++, GPIO_BASE | GPIO1_LO );
		S626MOD_WriteMem( pRPS++, RPS_NOP );							// VERSION 2.03 CHANGE: STRETCH OUT ADC START PULSE.
		S626MOD_WriteMem( pRPS++, RPS_LDREG | (P_GPIO >> 2) );		// End ADC Start pulse.
		S626MOD_WriteMem( pRPS++, GPIO_BASE | GPIO1_HI );
		...
		S626MOD_WriteMem( pRPS++, RPS_PAUSE | RPS_GPIO2 );			// Wait for ADC done.

		// Transfer ADC data from FB BUFFER 1 register to MyADCdata[] DMA buffer.
		S626MOD_WriteMem( pRPS++, RPS_STREG | ( BUGFIX_STREG( P_FB_BUFFER1 ) >> 2 ) );
		S626MOD_WriteMem( pRPS++, (U32)c->ANABuf.PhysicalBase + ( c->AdcItems << 2 ) );
	...
	S626MOD_WriteMem( pRPS++, RPS_CLRSIGNAL | RPS_SIGADC );					// Signal ReadADC() that scan is done.

	// Restart RPS program at its beginning.
	S626MOD_WriteMem( pRPS++, RPS_JUMP );									// Branch to start of RPS program.
	S626MOD_WriteMem( pRPS++, (U32)c->RPSBuf.PhysicalBase );
...
static void InitADC( struct CORE_OBJ *c )
{
	...
	WR7146( P_RPSADDR1, (U32)c->RPSBuf.PhysicalBase );	// Physical start of RPS program.
...
static void InitDAC( struct CORE_OBJ *c )
{
	...
	WR7146( P_BASEA2_OUT, (U32)pPhysBuf );				// Buffer base adrs.
	WR7146( P_PROTA2_OUT, (U32)( pPhysBuf + 1 ) );		// Protection address.
...
U32 S626CORE_CounterReadLatch( HBD bd, U16 chan )
{
	struct CORE_OBJ *c;
	U32 rtnval = 0;
