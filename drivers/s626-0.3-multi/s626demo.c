...
{
	int low, high;
	unsigned long		busNum = 5;					// All PCI boards on Bus 5
	unsigned long		slotNum = {1,2};
	int               	i, ch;
	unsigned int		useBoard = 1;				// Number of boards to use
	unsigned int		board;
	char				cmd[MAXLEN];
	char				arg[MAXLEN];
	char				*c;
	...
	for (board = 0; board <= useBoard; ++board)
	{
		S626_OpenBoard( board, 0, 1, InterruptAppISR, board );
		...
		if (errFlags!=0x0) 
		{
			printf("Board open/installation with Err = 0x%x : ", errFlags);
			switch (errFlags)
			{
				case ERR_OPEN:
		...
		if ( fgets (cmdbuff, MAXBUF, stdin) != NULL )
		{
			...
			if ( cmdbuff[0] != 0 )
			{
				...
				// for other commands.
					...
					SetDIO( board );	// set digital output.
					continue;
				} else if ( strcmp( cmd, "so" ) == 0 ) {	// shortcut of "sdo"
					SetDIO( board );	// set digital output.
					continue;
				} else if ( strcmp( cmd, "cdo" ) == 0 ) {
					ClearDIO( board );	// clear digital output.
					continue;
				} else if ( strcmp( cmd, "co" ) == 0 ) {	// shortcut of "cdo"
					ClearDIO( board );	// clear digital output.
					continue;
					...
					continue;
				} else if ( strcmp( cmd, "rd" ) == 0 ) {	// shortcut of "rpda"
					RampDAC( board );		// ramp DAC output.
					continue;
				} else if ( strcmp( cmd, "outda" ) == 0 ) {
					OutDAC( board );		// DAC output.
					continue;
				} else if ( strcmp( cmd, "od" ) == 0 ) {	// shortcut of "outda"
					OutDAC( board );		// DAC output.
					continue;
...
void MainMenu (void)
{
  ...
  printf ("\n==============================================================================\n\n"
			...
			"   sdO ......... set digital output ( DIO: 47-0 )                               \n"
			"   cdO ......... clear digital output ( DIO: 47-0 )                             \n"
			...
			"   outDA ....... DAC output ( channel: 0-3, -10V_/+10V )                        \n"
...
//*************************** OutDAC ****************************
//*  Ramp a selected DAC channel of board 0.
//*  output a voltage that will match the user voltage request.
//****************************************************************
...
void OutDAC( DWORD board )
{
  ...
  printf ("Enter Channel # and voltage [0-3 %+2.1f < voltage < %+2.1f, q] : ", min, max );

  for (;;)
  {
	  fgets (cmdbuff, MAXBUF, stdin);
	  if (cmdbuff[0] == 'q') return;
	  sscanf( cmdbuff, "%d %f", &chan, &volts );
	  if ( chan<0 || chan>3 )
	  {
		  printf ("Enter Channel # and voltage [0-3 %+2.1f < voltage < %+2.1f, q] : ", min, max );
		  continue;
...
#endif
//#if 0    	// jump-rampimg: step = 819
      temp = (int)(volts * 819.1);
      i = (short)temp;
      S626_WriteDAC (board, chan, i);
      printf ("\tChannel# %d :  DAC output = %5i ( %f V ) \n", chan, i, 10.0*(float)(i)/8191.0);
...
void SetDIO( DWORD board )
{
	...
	printf ("Enter Digital Out # to set [0-47, q] : ");

	for (;;)
	{
		fgets (cmdbuff, MAXBUF, stdin);
		if (cmdbuff[0] == 'q') return;
		sscanf( cmdbuff, "%d", &out);
		if ( out<0 || out>47 )
		{
			printf ("Enter Digital Out # to set [0-47, q] : ");
			continue;
		...
		else
		{
			if ( out<16 )
			{
				temp = 1;
				data1 |= temp << out;
				S626_DIOWriteBankSet (board, 0, data1);	//channels 1-16
			}
			else if( out<32 )
			{
				temp = 1;
				data2 |= temp << (out-16);
				S626_DIOWriteBankSet (board, 1, data2);	//channels 17-32
			}
			else if( out<48 )
			{
				temp = 1;
				data3 |= temp << (out-32);
				S626_DIOWriteBankSet (board, 2, data3);	//channels 33-48printf ("Enter Digital Out # [0-47, q] : ");
			}
	
			printf ("\n"	//Press ESC to return to the main menu.\n\n"
			"47               32   31               16   15                0   (DIO #)\n");
	
			HexBin (data3);		//convert data to binary & print it
			printf ("- ");
			HexBin (data2);		//convert data to binary & print it
			printf ("- ");
			HexBin (data1);		//convert data to binary & print it
			printf ("\n");
...
//**********************ClearDIO********************************
//** Clear digital output on board 0
//***************************************************************
...
void ClearDIO( DWORD board )
{
	...
	printf ("Enter Digital Out # to clear [0-47, q] : ");
...
void IntTest( DWORD board )
{
  ...
  // Enable interrupt to start the testprintf ("Enter Digital Out # [0-47, q] : ");
...
{
  BYTE                  poll_list[16];		// used to setup the ADC
  WORD                  data[10000];		// holds the values read by the ADC
  int                   chan, gain;
  int                   ch;
  int					i;
...
void MeasureDI( DWORD board )
{
  ...
  WORD              data[max];			// holds the values read from digital input.
  long int			i, times;
...
void QuadratureTest( DWORD board )
{
	WORD            Counter = CNTR_0B;	// CNTR_0A:	inputs on J5: CNTR_0A - A-,A+,B-,B+,I-,I+
										// CNTR_1A:	inputs on J5: CNTR_1A - A-,A+,B-,B+,I-,I+
										// CNTR_2A:	inputs on J5: CNTR_2A - A-,A+,B-,B+,I-,I+ 
										// CNTR_0B:	inputs on J4: CNTR_0B - A-,A+,B-,B+,I-,I+ 
										// CNTR_1B:	inputs on J4: CNTR_1B - A-,A+,B-,B+,I-,I+
