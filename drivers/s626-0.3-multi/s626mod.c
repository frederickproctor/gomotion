...
// Revision  :
//             eb., 2004   Charlie X. Liu initial
//            Mar., 2004   Charlie X. Liu fixes for MOD-level
//            object constructor and S626_OpenBoard()
//
// Copyright : (C) Sensoray 2004
//
// $Log: s626mod.c,v $
// Revision 1.1  2006/12/07 13:52:44  proctor
// Initial checkin of S626 multi-board fix from ATR
//
//
///////////////////////////////////////////////////////////////////////////////
...
// Size of SAA7146 application memory-mapped I/O space (PCI region 0)
// occupied by the 626 board.
#define REGION_SIZE_626	512

// Specific for 626
// Declare as a RevA model 626 board.
#define REV_A			TRUE

// Declare as a RevB, or higher, model 626 board.
#define REV_B			FALSE

// Attach to board only if PCI sub-IDs are valid.
#define SUBID_626		TRUE

// Attach to any board with an SAA7146 bridge.
#define SUBID_ANY		FALSE
...
typedef struct MOD_OBJ {
	...
	void				*IntCallback;			// Address of application interrupt
												// callback function.
	SX_CARD_REGISTER	CardReg;				// Board attributes.
	DWORD				IntPriority;			// Priority of interrupt thread.
	pthread_mutex_t		CriticalSection;		// Synchronization structure for
												// thread-safe execution.
	pthread_mutex_t		CriticalSectionAdc;		// Synchronization structure for
												// thread-safe ADC execution.
	HANDLE				hThreadEvent;			// Sync object for main/interrupt
												// threads.
	pthread_t			hIntThread;				// Handle to interrupt thread.
	BOOLEAN				IsIntStopped;			// Interrupt service has been
												// terminated.
	SX_INTERRUPT		Intrpt;					// WinDrvr interrupt control/status
												// structure.
	DWORD				CallbackArg;			// Interrupt callback argument.
	void				*IntCallbackArg;		// Pointer to interrupt callback argument.

// pointers for Diagnostics functions:

// for S626_I2CRead().
unsigned char ( *I2Cread )( HBD, unsigned char );

// for S626_I2CWrite().
void ( *I2Cwrite )( HBD, unsigned char, unsigned char );

// for S626_RegRead().
unsigned short ( *DEBIread )( HBD, unsigned short );

// for S626_RegWrite().
void ( *DEBIwrite )( HBD, unsigned short, unsigned short );

// pointers for Analog I/O functions:
// for S626_ResetADC().
void ( *ResetADC )( HBD, unsigned char * );

// for S626_ReadADC().
void ( *ReadADC )( HBD, unsigned short * );

// for S626_SingleADC().
unsigned short ( *SingleADC )( HBD, unsigned short );
// for S626_StartADC().
void ( *StartADC )( HBD );
// for S626_WaitDoneADC().
void ( *WaitDoneADC )( HBD, unsigned short *);
// for S626_WriteDAC().
void ( *WriteDAC )( HBD, unsigned short, short );
// for S626_WriteTrimDAC().
void ( *WriteTrimDAC )( HBD, unsigned char, unsigned char );

// pointers for Battery functions:
// for S626_BackupEnableGet().
unsigned short ( *BackupEnableGet )	( HBD );
// for S626_BackupEnableSet().
void ( *BackupEnableSet )( HBD, unsigned short );
// for S626_ChargeEnableGet().
unsigned short ( *ChargeEnableGet )	( HBD );
// for S626_ChargeEnableSet().
void ( *ChargeEnableSet )( HBD, unsigned short );

// pointers for Watchdog functions:
// for S626_WatchdogTimeout().
unsigned short ( *WatchdogTimeout )( HBD );
// for S626_WatchdogEnableGet().
unsigned short ( *WatchdogEnableGet )( HBD );
// for S626_WatchdogEnableSet().
void ( *WatchdogEnableSet )( HBD, unsigned short );
// for S626_WatchdogPeriodGet().
unsigned short ( *WatchdogPeriodGet )( HBD );
// for S626_WatchdogPeriodSet().
void ( *WatchdogPeriodSet )( HBD, unsigned short );
// for S626_WatchdogReset().
void ( *WatchdogReset )( HBD );

// pointers for Digital I/O functions:
// for S626_DIOReadBank().
unsigned short ( *DIOGroupRead )( HBD, unsigned short );
// for S626_DIOWriteBankGet().
unsigned short ( *DIOGroupWriteGet )( HBD, unsigned short );
// for S626_DIOWriteBankSet().
void ( *DIOGroupWriteSet )( HBD, unsigned short, unsigned short );
// for S626_DIOEdgeGet().
unsigned short ( *DIOGroupEdgeGet )( HBD, unsigned short );
// for S626_DIOEdgeSet().
void ( *DIOGroupEdgeSet )( HBD, unsigned short, unsigned short );
// for S626_DIOCapEnableSet().
void ( *DIOGroupCapEnableSet )( HBD, unsigned short, unsigned short, unsigned short );
// for S626_DIOCapEnableGet().
unsigned short ( *DIOGroupCapEnableGet )( HBD, unsigned short );
// for S626_DIOCapStatus().
unsigned short ( *DIOGroupCapStatus )( HBD, unsigned short );
// for S626_DIOCapReset().
void ( *DIOGroupCapReset )( HBD, unsigned short, unsigned short );
// for S626_DIOIntEnableGet().
unsigned short ( *DIOGroupIntEnableGet )( HBD, unsigned short );
// for S626_DIOIntEnableSet().
void ( *DIOGroupIntEnableSet )( HBD, unsigned short, unsigned short );
// for S626_DIOModeGet().
unsigned short ( *DIOChanModeGet )( HBD, unsigned short );
// for S626_DIOModeSet().
void ( *DIOChanModeSet )( HBD, unsigned short, unsigned short );

// pointers for Counter functions:
// for S626_CounterReadLatch().
unsigned long ( *CounterReadLatch )( HBD, unsigned short );
// for S626_CounterCapFlagsReset().
void ( *CounterResetCapFlags )( HBD, unsigned short );
// for S626_CounterCapStatus().
unsigned short ( *CounterCapStatus )( HBD );
// for S626_CounterModeGet().
unsigned short ( *CounterModeGet )( HBD, unsigned short );
// for S626_CounterModeSet().
void ( *CounterModeSet )( HBD, unsigned short, unsigned short );
// for S626_CounterLatchSourceSet().
void ( *CounterLatchSrcSet )( HBD, unsigned short, unsigned short );
// for S626_CounterEnableSet().
void ( *CounterEnableSet )( HBD, unsigned short, unsigned short );
// for S626_CounterLoadTrigSet().
void ( *CounterLoadTrigSet )( HBD, unsigned short, unsigned short );
// for S626_CounterSoftIndex().
void ( *CounterSoftIndex )( HBD, unsigned short );
// for S626_CounterPreload().
void ( *CounterPreload )( HBD, unsigned short, unsigned long );
// for S626_CounterIntSourceSet().
void ( *CounterIntSourceSet )( HBD, unsigned short, unsigned short );

// pointers for Status and control functions:
// for S626_GetAddress().
unsigned long ( *GetAddress )( HBD );
// for S626_GetErrors().
unsigned long ( *GetErrors )( HBD );
// for S626_SetErrCallback().
void ( *SetErrCallback )( HBD, FPTR_ERR );
// for S626_InterruptStatus().
void ( *InterruptStatus )( HBD, unsigned short * );
// for S626_InterruptEnable().
void ( *InterruptEnable )( HBD, unsigned short );
} MOD_OBJ;
...
DWORD S626MOD_ReadMem( /*HBD bd,*/ DWORD *address )
{
	DWORD			prmt;
	//struct MOD_OBJ	*m = &mo[bd];
	struct MOD_OBJ	*m = &mo[0];	// let's always use first handle to execute reading,
									// since it doesn't matter which one to use
									// and it makes core simpler.

	// return (volatile)*address;
	//return readl( addr );
	prmt = (DWORD) address;			// pass-in address.
	if ( ioctl( m->hSX, S626_IOC_ReadRegister, &prmt ) == 0 )
		return prmt;				// return-back data
	else
		return 0xffffffff;
...
void S626MOD_WriteMem( /*HBD bd,*/ DWORD *address, DWORD data )
{
	ioc_param		prmt;
	//struct MOD_OBJ	*m = &mo[bd];
...
//////////////////////////////////////////////////////////////////////////////
// Interrupt thread entry point.  The interrupt handler executes on a private
// thread as it must suspend execution while waiting for WinDrvr to receive an
// interrupt request.  The thread entry point is implemented as a friend
// function because _beginthread() requires that any function launched on a
// new thread must be _cdecl, which is not compatible with class member functions.
...
static PVOID IntWait( PVOID pobj )
{
	...
	for ( ; ; )
	{
		// Suspend this thread until either (1) IRQ occurs, or (2) interrupt
		...
		//this call blocks until the interrupt happens
		result = ioctl( m->hSX, S626_IOC_InterruptOn, &m->Intrpt );	
		...
		//if ( m->IntCallbackArg )
		//	( ( void (*)(void *) )m->IntCallback )( (void *)m->IntCallbackArg );// Callback is passed a pointer.
		//else

		( (FPTR_ISR)m->IntCallback )(m->CallbackArg);	// Callback expects no arguments.
...
static void LockBoard( struct MOD_OBJ *m, SX_PCI_SLOT *Loc, BOOLEAN MatchSubIDs, BYTE IsBoardRevA )
{
    ...
    S626CORE_SetBase( m->Board, pItemMemory->I.Mem.dwUserDirectAddr );		// Direct access base address.
	
	// Save bus number and slot number of card.
	S626CORE_SetAddress( m->Board, ( Loc->dwBus << 16 ) | Loc->dwSlot );

	printf("Board(%d) - Address: 0x%x\n", m->Board, ( Loc->dwBus << 16 ) | Loc->dwSlot);
...
/////////////////////////////////////////////////////////////////////////////////////
// Register first available 626 board at any location.  If MatchSubIDs is asserted,
...
static void RegisterAny( struct MOD_OBJ *m, BOOLEAN MatchSubIDs, BYTE IsBoardRevA )
{
	...
	//PciScan.searchId.dwVendorId = SUBVENDORID626;
	...
	// If at least one matching card was found, register the first board that is not locked.
...
static void Constructor( HBD bd, DWORD PhysLoc, DWORD Priority, VOID *callback_fn, DWORD callback_arg )
{
	struct MOD_OBJ		*m = &mo[bd];
	...
	m->IntCallbackArg = &m->CallbackArg;// Create pointer to interrupt callback argument.
	...
	// The physical location of the 626 board is not specified,
	// so we will try to register the first available board, declared as RevA,
	// that has any PCI sub-IDs.  If no such board is found, the registration will fail.
	// This option is used for accessing RevA or RevAB boards that have any PCI sub-IDs.
	// If the registration fails, the board is either RevB, or it is a RevA/RevAB and
	...
	// The physical location of the 626 board is not specified,
	// so we will try to register the first available board, declared as RevA,
	// that has valid PCI sub-IDs.  If no such board is found, the registration will fail.
	// This option is used for accessing RevA or RevAB boards that have valid PCI sub-IDs.
	// If the registration fails, the board is either RevB, or it is a RevA and
	...
	// This option is for Sensoray internal use only; it is not intended for use by customers.
	// The physical location of the 626 board is not specified,
	// so we will try to register the first available board, declared as RevB,
	// that has valid PCI sub-IDs.  If no such board is found, the registration will fail.
	// This option is used for accessing ONLY RevB/RevAB boards that have valid PCI sub-IDs.
	// If the registration fails, the board is either RevA, or it is a RevB/RevAB and
	// the EEPROM has malfunctioned or has not yet had the PCI sub-IDs programmed into it.
	...
	// The physical location of the 626 board is not specified,
	// so we will try to register the first available board, declared as RevB,
	// that has an SAA7146 bridge device.  If no such board is found, the registration
	// will fail.  This option is used for registering RevB/RevAB boards before the EEPROM
	// has been programmed with valid sub-IDs.  If a RevA board has been registered, this
	// can be detected by attempting to access the EEPROM; the EEPROM will be accessible if
	// the board is RevB, but not if it is a RevA.
	...
	// first available board, declared as RevB, that has valid PCI sub-IDs.
		RegisterAny( m, SUBID_626, REV_B );
	// If no matching boards were found, we will now try to register the
...
void S626MOD_AllocDMAB( HBD bd, DWORD nBytes, DMABUF *BdDmaBuf )
{
	...
	dma_arg.data = nBytes;
	dma_arg.address = (DWORD) m->CardReg.hCard;
...
// Diagnostics:
unsigned char S626_I2CRead(HBD hbd, unsigned char addr)
{
	return BD.I2Cread(hbd, addr);
}
void S626_I2CWrite(HBD hbd, unsigned char addr, unsigned char data)
{
	BD.I2Cwrite(hbd, addr, data);
}
unsigned short S626_RegRead(HBD hbd, unsigned short addr)
{
	return BD.DEBIread(hbd, addr);
}
void S626_RegWrite(HBD hbd, unsigned short addr, unsigned short data)
{ BD.DEBIwrite(hbd, addr, data);
}

// Analog I/O functions:
void S626_ResetADC(HBD hbd, unsigned char *ppl)
{
	BD.ResetADC(hbd, ppl);
}
void S626_ReadADC(HBD hbd, unsigned short *pdata)
{
	BD.ReadADC(hbd, pdata);
}
unsigned short S626_SingleADC(HBD hbd, unsigned short spec)
{
	return BD.SingleADC(hbd, spec);
}
void S626_StartADC(HBD hbd)
{
	BD.StartADC(hbd);
}
void S626_WaitDoneADC(HBD hbd, unsigned short *pdata)
{
	BD.WaitDoneADC(hbd, pdata );
}
void S626_WriteDAC(HBD hbd, unsigned short ch, long val)
{
	BD.WriteDAC(hbd, ch, (short)(val & 0xFFFF));
}
void S626_WriteTrimDAC(HBD hbd, unsigned char ch, unsigned char val)
{
	BD.WriteTrimDAC(hbd, ch, val);
}

// Battery functions:
unsigned short S626_BackupEnableGet(HBD hbd)
{
	return BD.BackupEnableGet(hbd);
}
void S626_BackupEnableSet(HBD hbd, unsigned short en)
{
	BD.BackupEnableSet(hbd, en);
}
unsigned short S626_ChargeEnableGet(HBD hbd)
{
	return BD.ChargeEnableGet(hbd);
}
void S626_ChargeEnableSet(HBD hbd, unsigned short en)
{
	BD.ChargeEnableSet(hbd, en);
}

// Watchdog functions:
unsigned short S626_WatchdogTimeout(HBD hbd)
{
	return BD.WatchdogTimeout(hbd);
}
unsigned short S626_WatchdogEnableGet(HBD hbd)
{
	return BD.WatchdogEnableGet(hbd);
}
void S626_WatchdogEnableSet(HBD hbd, unsigned short en)
{
	BD.WatchdogEnableSet(hbd, en);
}
unsigned short S626_WatchdogPeriodGet(HBD hbd)
{
	return BD.WatchdogPeriodGet(hbd);
}
void S626_WatchdogPeriodSet(HBD hbd, unsigned short val)
{
	BD.WatchdogPeriodSet(hbd, val);
}
void S626_WatchdogReset	(HBD hbd)
{
	BD.WatchdogReset(hbd);
}

// Digital I/O functions:
unsigned short S626_DIOReadBank	(HBD hbd, unsigned short grp)
{
	return BD.DIOGroupRead(hbd, grp);
}
unsigned short S626_DIOWriteBankGet(HBD hbd, unsigned short grp)
{
	return BD.DIOGroupWriteGet(hbd, grp);
}
void S626_DIOWriteBankSet(HBD hbd, unsigned short grp, unsigned short val)
{
	BD.DIOGroupWriteSet(hbd, grp, val);
}
unsigned short S626_DIOEdgeGet(HBD hbd, unsigned short grp)
{
	return BD.DIOGroupEdgeGet(hbd, grp);
}
void S626_DIOEdgeSet(HBD hbd, unsigned short grp, unsigned short val)
{
	BD.DIOGroupEdgeSet(hbd, grp, val);
}
void S626_DIOCapEnableSet(HBD hbd, unsigned short grp, unsigned short val, unsigned short en)
{
	BD.DIOGroupCapEnableSet(hbd, grp, val, en);
}
unsigned short S626_DIOCapEnableGet(HBD hbd, unsigned short grp)
{
	return BD.DIOGroupCapEnableGet(hbd, grp);
}
unsigned short S626_DIOCapStatus(HBD hbd, unsigned short grp)
{
	return BD.DIOGroupCapStatus(hbd, grp);
}
void S626_DIOCapReset(HBD hbd, unsigned short grp, unsigned short val)
{
	BD.DIOGroupCapReset(hbd, grp, val);
}
unsigned short S626_DIOIntEnableGet(HBD hbd, unsigned short grp)
{
	return BD.DIOGroupIntEnableGet(hbd, grp);
}
void S626_DIOIntEnableSet(HBD hbd, unsigned short grp, unsigned short val)
{
	BD.DIOGroupIntEnableSet(hbd, grp, val);
}
unsigned short S626_DIOModeGet(HBD hbd, unsigned short grp)
{
	return BD.DIOChanModeGet(hbd, grp);
}
void S626_DIOModeSet(HBD hbd, unsigned short grp, unsigned short val)
{
	BD.DIOChanModeSet(hbd, grp, val);
}

// Counter functions:
unsigned long S626_CounterReadLatch(HBD hbd, unsigned short ch)
{
	return BD.CounterReadLatch(hbd, ch);
}
void S626_CounterCapFlagsReset	(HBD hbd, unsigned short ch)
{
	BD.CounterResetCapFlags(hbd, ch);
}
unsigned short S626_CounterCapStatus(HBD hbd)
{
	return BD.CounterCapStatus(hbd);
}
unsigned short S626_CounterModeGet(HBD hbd, unsigned short ch)
{
	return BD.CounterModeGet(hbd, ch);
}
void S626_CounterModeSet(HBD hbd, unsigned short ch, unsigned short opt)
{
	BD.CounterModeSet(hbd, ch, opt);
}
void S626_CounterLatchSourceSet(HBD hbd, unsigned short ch, unsigned short val)
{
	BD.CounterLatchSrcSet(hbd, ch, val);
}
void S626_CounterEnableSet(HBD hbd, unsigned short ch, unsigned short en)
{
	BD.CounterEnableSet(hbd, ch, en);
}
void S626_CounterLoadTrigSet(HBD hbd, unsigned short ch, unsigned short trg)
{
	BD.CounterLoadTrigSet(hbd, ch, trg);
}
void S626_CounterSoftIndex(HBD hbd, unsigned short ch)
{
	BD.CounterSoftIndex(hbd, ch);
}
void S626_CounterPreload(HBD hbd, unsigned short ch, unsigned long val)
{
	BD.CounterPreload(hbd, ch, val);
}
void S626_CounterIntSourceSet(HBD hbd, unsigned short ch, unsigned short src)
{
	BD.CounterIntSourceSet(hbd, ch, src);
}

// Status and control functions:
unsigned long S626_GetAddress(HBD hbd)
{
	return BD.GetAddress(hbd);
}
unsigned long S626_GetErrors(HBD hbd)
{
	return BD.GetErrors(hbd);
}
void S626_SetErrCallback(HBD hbd, FPTR_ERR Callback)
{
	BD.SetErrCallback(hbd, Callback);
}
void S626_InterruptStatus(HBD hbd, unsigned short *IntFlags)
{
	BD.InterruptStatus(hbd, IntFlags);
}
void S626_InterruptEnable(HBD hbd, unsigned short en)
{
	BD.InterruptEnable(hbd, en);
}
...
void S626_CloseBoard( HBD hbd )
{
	...
	// including: Close the board at CORE level, using S626CORE_BoardClose( hbd );
	Destructor( hbd );
...
void S626_OpenBoard( HBD hbd, unsigned long PhysLoc, unsigned long Priority,
FPTR_ISR Callback, unsigned long CallbackArg)
{
	// Abort if a valid board handle is not specified.
	if ( hbd >= MAX_BOARDS )
		return;
	...
	// including: Close the board if it is open at CORE level, using
	// S626CORE_BoardClose( hbd );
	// Destructor( hbd );	
	...
	// including: Open the board at CORE level, using
	// S626CORE_BoardOpen( hbd, IsBoardRevA );
	Constructor( hbd, PhysLoc, Priority, Callback, CallbackArg);

	//if ( S626CORE_GetErrors( HBD bd ) &
	// ( ERR_OPEN | ERR_CARDREG | ERR_ALLOC_MEMORY |
	// ERR_LOCK_BUFFER | ERR_THREAD | ERR_INTERRUPT ) )
