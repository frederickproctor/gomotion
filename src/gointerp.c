...
go_result go_interp_init(go_interp * i)
{
  int t;

  for (t = 0; t < GO_INTERP_COEFF_MAX; t++) {
    i->a.a[t] = 0.0;
    i->p.p[t] = 0.0;
  }
...
go_result go_interp_set_here(go_interp * i, go_real here)
{
  int t;
...
go_result go_interp_calc_coeff_constant(const go_interp_points * p,
					go_interp_coeff * a)
{
  a->a[0] = p->p[0];
...
go_result go_interp_add_constant(go_interp * i, go_real pt)
{
  /* replace current point */
  i->p.p[0] = pt;
  /* compute new coeff */
  return go_interp_calc_coeff_constant(&i->p, &i->a);
}

go_real go_interp_eval_constant(const go_interp * i, go_real t)
{
  return i->a.a[0];
}
...
go_result go_interp_calc_coeff_linear(const go_interp_points * p,
				      go_interp_coeff * a)
{
  a->a[0] = p->p[0];
  a->a[1] = p->p[1] - p->p[0];
...
go_result go_interp_add_linear(go_interp * i, go_real pt)
{
  /* shift points down */
  i->p.p[0] = i->p.p[1];
  i->p.p[1] = pt;
  /* compute new coeff */
  return go_interp_calc_coeff_linear(&i->p, &i->a);
}

go_real go_interp_eval_linear(const go_interp * i, go_real t)
{
  return i->a.a[0] + i->a.a[1] * t;
}
...
go_result go_interp_calc_coeff_cubic_bc(const go_interp_points * p,
					go_interp_coeff * a)
{
  go_real X;
  go_real Y;
...
go_result go_interp_calc_coeff_cubic_pf(const go_interp_points * p,
					go_interp_coeff * a)
{
  go_real X;
  go_real Y;
...
*/
go_result go_interp_add_cubic_pv(go_interp * i, go_real pos, go_real vel)
{
  /* shift p-v pair down */
  i->p.p[0] = i->p.p[2];
  i->p.p[1] = i->p.p[3];
  ...
  i->p.p[3] = vel;
  /* compute new coeff */
  return go_interp_calc_coeff_cubic_bc(&i->p, &i->a);
}
 ...
 */
go_result go_interp_add_cubic_pdv(go_interp * i, go_real pt)
{
  go_interp_points p;
  go_real v0, v1;
  go_real wp;
  ...
  v0 = 0.5 * (i->p.p[2] - i->p.p[0]);
  v1 = 0.5 * (pt - i->p.p[1]);
  /* set up point structure */
  wp = i->p.p[1];		/* original */
  wp = kp2_inv * (i->p.p[0] + k * i->p.p[1] + i->p.p[2]);	/* blended */
  p.p[0] = wp;
  p.p[1] = v0;
  wp = i->p.p[2];		/* original */
  wp = kp2_inv * (i->p.p[1] + k * i->p.p[2] + pt);	/* blended */
  p.p[2] = wp;
  p.p[3] = v1;
  /* shift points down */
  i->p.p[0] = i->p.p[1];
  ...
  return go_interp_calc_coeff_cubic_bc(&p, &i->a);
}
...
/*
  ...
  will fill the interpolator and make it ready to go.
*/
go_result go_interp_add_cubic_pf(go_interp * i, go_real pt)
{
  /* shift points down */
  i->p.p[0] = i->p.p[1];
  i->p.p[1] = i->p.p[2];
  ...
  i->p.p[3] = pt;
  /* compute new coeff */
  return go_interp_calc_coeff_cubic_pf(&i->p, &i->a);
}

/*
  Cubic interpolator, same for both boundary- and point-fit cubics
*/
go_real go_interp_eval_cubic(const go_interp * i, go_real t)
{
  return ((i->a.a[3] * t + i->a.a[2]) * t + i->a.a[1]) * t + i->a.a[0];
}
...
go_result go_interp_calc_coeff_quintic_bc(const go_interp_points * p,
					  go_interp_coeff * a)
{
  go_real X;
  go_real Y;
...
go_result go_interp_calc_coeff_quintic_pf(const go_interp_points * p,
					  go_interp_coeff * a)
{
  go_real X;
  go_real Y;
...
*/
go_result go_interp_add_quintic_pva(go_interp * i, go_real pos, go_real vel,
				    go_real acc)
{
  /* shift p-v pair down */
  i->p.p[0] = i->p.p[3];
  ...
  i->p.p[5] = acc;
  /* compute new coeff */
  return go_interp_calc_coeff_quintic_bc(&i->p, &i->a);
}
 ...
 */
go_result go_interp_add_quintic_pvda(go_interp * i, go_real pos, go_real vel)
{
  go_interp_points p;
  go_real a0, a1;

  /* estimate boundary values */
  a0 = 0.5 * (i->p.p[5] - i->p.p[1]);
  a1 = 0.5 * (vel - i->p.p[3]);
  ...
  p.p[4] = i->p.p[5];
  p.p[5] = a1;
  /* shift pairs down, ignoring the unused i->p.p[0] */
  i->p.p[1] = i->p.p[3];
  i->p.p[2] = i->p.p[4];
  i->p.p[3] = i->p.p[5];
  /* add new pair */
  ...
  i->p.p[5] = vel;
  /* compute new coeff */
  return go_interp_calc_coeff_quintic_bc(&p, &i->a);
}
 ...
 */
go_result go_interp_add_quintic_pdva(go_interp * i, go_real pos)
{
  go_interp_points p;
  go_real v0, v1, a0, a1;
  go_real wp;
  ...
  /* estimate boundary values */
  v0 = 0.5 * (i->p.p[3] - i->p.p[1]);
  v1 = 0.5 * (i->p.p[4] - i->p.p[2]);
  a0 = 0.25 * (i->p.p[4] - i->p.p[2] - i->p.p[2] + i->p.p[0]);
  a1 = 0.25 * (pos - i->p.p[3] - i->p.p[3] + i->p.p[1]);
  /* set up p-v-a structure */
  wp =
    (-0.086 * i->p.p[0] + 0.343 * i->p.p[1] + 0.486 * i->p.p[2] +
     0.343 * i->p.p[3] + -0.086 * i->p.p[4]);
  wp = (i->p.p[1] + 2.5 * i->p.p[2] + i->p.p[3]) / 4.5;
  wp = i->p.p[2];		/* original */
  p.p[0] = wp;
  p.p[1] = v0;
  p.p[2] = a0;
  wp =
    (-0.086 * i->p.p[1] + 0.343 * i->p.p[2] + 0.486 * i->p.p[3] +
     0.343 * i->p.p[4] + -0.086 * pos);
  wp = (i->p.p[2] + 2.5 * i->p.p[3] + i->p.p[4]) / 4.5;
  wp = i->p.p[3];		/* original */
  p.p[3] = wp;
  p.p[4] = v1;
  p.p[5] = a1;
  /* shift points down, ignoring unused i->p.p[5] */
  i->p.p[0] = i->p.p[1];
  i->p.p[1] = i->p.p[2];
  i->p.p[2] = i->p.p[3];
  i->p.p[3] = i->p.p[4];
...
*/
go_result go_interp_add_quintic_pf(go_interp * i, go_real pos)
{
  /* shift points down */
  i->p.p[0] = i->p.p[1];
  i->p.p[1] = i->p.p[2];
  ...
  i->p.p[5] = pos;
  /* compute new coeff */
  return go_interp_calc_coeff_quintic_pf(&i->p, &i->a);
}

/*
  Quintic interpolator, same for both boundary- and point-fit quintics
*/
go_real go_interp_eval_quintic(const go_interp * i, go_real t)
{
  return ((((i->a.a[5] * t + i->a.a[4]) * t + i->a.a[3]) * t + i->a.a[2]) *
	  t + i->a.a[1]) * t + i->a.a[0];
}
