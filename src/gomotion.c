...
void go_position_zero_joints(go_position * position)
{
  go_integer i;
...
void go_position_zero_pose(go_position * position)
{
  position->u.pose.tran.x = 0.0;
  position->u.pose.tran.y = 0.0;
  position->u.pose.tran.z = 0.0;
  position->u.pose.rot.s = 1.0;
  position->u.pose.rot.x = 0.0;
  position->u.pose.rot.y = 0.0;
  position->u.pose.rot.z = 0.0;
}

go_result go_motion_spec_init(go_motion_spec * spec)
{
  /*
...
go_result go_motion_spec_set_type(go_motion_spec * spec, go_flag type)
{
...
go_flag go_motion_spec_get_type(go_motion_spec * spec)
{
  return spec->type;
}

go_result go_motion_queue_set_joint_number(go_motion_queue * queue,
					   go_integer joints)
{
  ...
  queue->joint_num = joints;

  return GO_RESULT_OK;
}

go_integer go_motion_queue_get_joint_number(go_motion_queue * queue)
{
  return queue->joint_num;
}

go_result go_motion_spec_set_id(go_motion_spec * spec, go_integer id)
{
  spec->id = id;
...
go_integer go_motion_spec_get_id(go_motion_spec * spec)
{
  return spec->id;
}

go_result go_motion_spec_set_jpar(go_motion_spec * spec, go_integer i, go_real vel, go_real acc, go_real jerk)
{
  ...
  spec->par[i].vel = vel, spec->par[i].acc = acc, spec->par[i].jerk = jerk;

  return GO_RESULT_OK;
}

go_result go_motion_spec_set_tpar(go_motion_spec * spec, go_real vel, go_real acc, go_real jerk)
{
  if (vel <= GO_REAL_EPSILON ||
      acc <= GO_REAL_EPSILON ||
      jerk <= GO_REAL_EPSILON) return GO_RESULT_BAD_ARGS;

  spec->par[0].vel = vel, spec->par[0].acc = acc, spec->par[0].jerk = jerk;

  return GO_RESULT_OK;
}

go_result go_motion_spec_set_rpar(go_motion_spec * spec, go_real vel, go_real acc, go_real jerk)
{
  if (vel <= GO_REAL_EPSILON ||
      acc <= GO_REAL_EPSILON ||
      jerk <= GO_REAL_EPSILON) return GO_RESULT_BAD_ARGS;

  spec->par[1].vel = vel, spec->par[1].acc = acc, spec->par[1].jerk = jerk;

  return GO_RESULT_OK;
}

go_result go_motion_spec_set_cpar(go_motion_spec * spec, go_cart * center, go_cart * normal, go_integer turns)
{
  go_result retval;

  spec->u.cpar.center = *center;
  retval = go_cart_unit(normal, &spec->u.cpar.normal);
  if (GO_RESULT_OK != retval) return retval;
  spec->u.cpar.turns = turns;

  return GO_RESULT_OK;
}

go_result go_motion_spec_set_time(go_motion_spec * spec, go_real time)
{
  if (time > 0.0) {
    spec->totalt = time;
    return GO_RESULT_OK;
...
go_result go_motion_spec_set_end_position(go_motion_spec * spec, go_position * end)
{
  spec->end = *end;
...
go_result go_motion_spec_set_end_pose(go_motion_spec * spec, go_pose * end)
{
  spec->end.u.pose = *end;
...
go_result go_motion_queue_init(go_motion_queue * queue,
			       go_motion_spec * space,
			       go_integer size,
			       go_real deltat)
{
  if (0 == queue || 0 == space || size <= 0 || deltat <= 0) {
    return GO_RESULT_BAD_ARGS;
  }

  queue->size = size;
  queue->startptr = space;
  queue->endptr = space + size;
  queue->deltat = deltat;
  queue->joint_num = GO_MOTION_JOINT_NUM;

  return go_motion_queue_reset(queue);
}

go_result go_motion_queue_reset(go_motion_queue * queue)
{
  queue->type = GO_MOTION_NONE;
  queue->start = queue->startptr;
  queue->end = queue->start;
  queue->number = 0;
  queue->last_id = 0;
  queue->time = 0.0;

  return go_scale_init(&queue->timescale, 1.0);
}

go_result go_motion_queue_set_type(go_motion_queue * queue, go_flag type)
{
...
go_flag go_motion_queue_get_type(go_motion_queue * queue)
{
  return queue->type;
}

go_result go_motion_queue_set_here(go_motion_queue * queue,
				   const go_position * here)
{
  ...
  if (queue->type == GO_MOTION_JOINT ||
      queue->type == GO_MOTION_UJOINT) {
    for (i = 0; i < queue->joint_num; i++) {
      queue->there.u.joint[i] = queue->here.u.joint[i] = here->u.joint[i];
    }
    return GO_RESULT_OK;
  }

  if (queue->type == GO_MOTION_WORLD) {
    queue->there.u.pose = queue->here.u.pose = here->u.pose;
    return GO_RESULT_OK;
  }
...
go_result go_motion_queue_set_cycle_time(go_motion_queue * queue,
					 go_real deltat)
{
  if (deltat <= 0.) {
    return GO_RESULT_BAD_ARGS;
...
go_result go_scale_init(go_scale_spec * spec, go_real scale)
{
  spec->scaling = 0;
  spec->scale = scale;
  spec->scale_dir = 0;
  spec->scale_b = spec->scale;
  spec->scale_next = spec->scale;
  spec->scale_v_next = 1.0;
  spec->scale_a_next = 1.0;
  spec->scale_t = 0.0;

  return GO_RESULT_OK;
}

go_result go_scale_set(go_scale_spec * spec, 
		       go_real scale,
		       go_real scale_v,
		       go_real scale_a)
{
  ...
  spec->scale_next = scale;
  /* need to save these so they can be used when loading scale_next */
  spec->scale_v_next = scale_v;
  spec->scale_a_next = scale_a;
  ...
  if (scale > spec->scale) {
    scale_incr = scale - spec->scale;
    spec->scale_dir = 1;
  } else {
    scale_incr = spec->scale - scale;
    spec->scale_dir = 0;
  }
  spec->scaling = 1;
  spec->scale_b = spec->scale;
  ...
  return go_traj_ca_compute(scale_incr, scale_v, scale_a, &spec->scale_spec);
}

go_result go_scale_eval(go_scale_spec * spec,
			go_real deltat, 
			go_real * scale)
{
  ...
  if (spec->scaling) {
    ...
    go_traj_ca_interp(&spec->scale_spec, spec->scale_t, &ti);
    scale_incr = ti.d;

    if (spec->scale_dir) {
      spec->scale = spec->scale_b + scale_incr;
    } else {
      spec->scale = spec->scale_b - scale_incr;
    }
    if (spec->scale_t >= spec->scale_spec.tend) {
      spec->scaling = 0;
      if (! GO_TRAN_CLOSE(spec->scale, spec->scale_next)) {
	go_scale_set(spec, spec->scale_next,
		     spec->scale_v_next, spec->scale_a_next);
      }
    }
  }
  if (NULL != scale) *scale = spec->scale;

  return GO_RESULT_OK;
}

go_result go_motion_queue_set_scale(go_motion_queue * queue,
				    go_real scale,
				    go_real scale_v,
				    go_real scale_a)
{
  return go_scale_set(&queue->timescale, scale, scale_v, scale_a);
}
...
/*
  ...
  spec.start.u.joint[] for the start positions (from queue->there.u.joint[]);
  ...
  queue->there.u.joint[] for the end positions (from spec.end.u.joint[]);
  the queue pointers for an append.
 */

static go_result
go_motion_queue_append_joint(go_motion_queue * queue,
			     const go_motion_spec * motion)
{
  ...
  for (i = 0; i < queue->joint_num; i++) {
    ...
    spec.start.u.joint[i] = queue->there.u.joint[i];

    /* 'td' is the incremental distance traveled by this joint */
    td = fabs(spec.end.u.joint[i] - queue->there.u.joint[i]);

    /* set the end joint values for the queue */
    queue->there.u.joint[i] = spec.end.u.joint[i];
  ...
  *queue->end = spec;
  queue->number++;
  queue->end++;
  if (queue->end >= queue->endptr) {
    queue->end = queue->startptr;
  }
  queue->last_id = spec.id;

  return GO_RESULT_OK;
}

static go_result
go_motion_queue_append_ujoint(go_motion_queue * queue,
			      const go_motion_spec * motion)
{
  go_motion_spec spec;
  go_real td, maxtd;
...
static go_result
go_motion_queue_append_world(go_motion_queue * queue,
			     const go_motion_spec * motion)
{
  go_motion_spec spec;
  go_quat quat;
  ...
  case GO_MOTION_LINEAR:
    go_cart_cart_sub(&spec.end.u.pose.tran, &queue->there.u.pose.tran, &cart);
    go_cart_mag(&cart, &td);
  ...
  case GO_MOTION_CIRCULAR:
    ...
    /* get the vector from the center to the start and project onto
       the normal plane */
    (void) go_cart_cart_sub(&queue->there.u.pose.tran, &spec.u.cpar.center, &rcs);
  ...
  if (GO_RESULT_OK != go_quat_inv(&queue->there.u.pose.rot, &quat)) {
    /* 'there' should always be invertible; if not it wasn't initialized
       to I, so do it here */
    quat = queue->there.u.pose.rot = go_quat_identity();
  }
  ...
  spec.start = queue->there;
  queue->there = spec.end;
  *queue->end = spec;
  queue->number++;
  queue->end++;
  if (queue->end >= queue->endptr) {
...
go_result
go_motion_queue_append(go_motion_queue * queue,
		       const go_motion_spec * motion)
{
  if (queue->type == GO_MOTION_JOINT) {
    return go_motion_queue_append_joint(queue, motion);
  }

  if (queue->type == GO_MOTION_UJOINT) {
    return go_motion_queue_append_ujoint(queue, motion);
  }

  if (queue->type == GO_MOTION_WORLD) {
    return go_motion_queue_append_world(queue, motion);
  }

  return GO_RESULT_ERROR;
}

go_result go_motion_queue_number(const go_motion_queue * queue,
				 go_integer * number)
{
  *number = queue->number;
...
go_result go_motion_queue_size(const go_motion_queue * queue,
			       go_integer * size)
{
  *size = queue->size;
...
go_result go_motion_queue_head(const go_motion_queue * queue,
...
go_result 
go_motion_queue_here(const go_motion_queue * queue, 
		     go_position * position)
{
  ...
  if (queue->type == GO_MOTION_JOINT ||
      queue->type == GO_MOTION_UJOINT) {
    for (i = 0; i < queue->joint_num; i++) {
      position->u.joint[i] = queue->here.u.joint[i];
    }
    return GO_RESULT_OK;
  }

  if (queue->type == GO_MOTION_WORLD) {
    position->u.pose = queue->here.u.pose;
    return GO_RESULT_OK;
  }
...
go_result 
go_motion_queue_there(const go_motion_queue * queue, 
		      go_position * position)
{
  go_integer i;
  ...
  if (queue->type == GO_MOTION_JOINT ||
      queue->type == GO_MOTION_UJOINT) {
    for (i = 0; i < queue->joint_num; i++) {
      position->u.joint[i] = queue->there.u.joint[i];
    }
    return GO_RESULT_OK;
  }

  if (queue->type == GO_MOTION_WORLD) {
    position->u.pose = queue->there.u.pose;
    return GO_RESULT_OK;
  }
...
static go_result
go_motion_queue_interp_joint(go_motion_queue * queue, 
			     go_real * joint)
{
  go_motion_spec spec;
  ...
  for (i = 0; i < queue->joint_num; i++) {
    go_traj_cj_interp(&spec.cj[i], queue->time, &tis);
    if (spec.end.u.joint[i] > spec.start.u.joint[i]) {
      joint[i] = queue->here.u.joint[i] = spec.start.u.joint[i] + tis.d; 
    } else {
      joint[i] = queue->here.u.joint[i] = spec.start.u.joint[i] - tis.d; 
    }
  }

  if (queue->time >= spec.totalt) {
    /* we finished this move, so take it off the queue and set
       the time to zero as the basis for the next move */
    go_motion_queue_delete(queue);
    queue->time = 0.;
  }
...
static go_result
go_motion_spec_interp_world(go_motion_spec * motion, go_real time, go_pose * pose)
{
  go_traj_interp_spec tis;
...
static go_result 
go_motion_queue_interp_world(go_motion_queue * queue, go_pose * pose)
{
  go_motion_spec motion;
  go_real time;
  go_result retval;
  ...
  retval = go_motion_spec_interp_world(&motion, time, pose);
  if (GO_RESULT_OK != retval) return retval;

  if (queue->time >= motion.totalt) {
    /* we finished this move */
    go_motion_queue_delete(queue);
    queue->time = 0.;
  }

  queue->here.u.pose = *pose;

  return GO_RESULT_OK;
}

go_result 
go_motion_queue_interp(go_motion_queue * queue, go_position * position)
{
  if (queue->number > 0) {
    /*
    ...
    */
    go_scale_eval(&queue->timescale, queue->deltat, NULL);
    queue->time += queue->deltat * queue->timescale.scale;
  }

  if (queue->type == GO_MOTION_JOINT ||
      queue->type == GO_MOTION_UJOINT) {
    return go_motion_queue_interp_joint(queue, position->u.joint);
  }

  if (queue->type == GO_MOTION_WORLD) {
    return go_motion_queue_interp_world(queue, &position->u.pose);
  }
...
go_result go_motion_queue_stop(go_motion_queue * queue)
{
  ...
  (void) go_motion_queue_drop_pending(queue);
  ...
  if (queue->type == GO_MOTION_JOINT) {
	...
	specptr->end.u.joint[i] = specptr->start.u.joint[i] + specptr->cj[i].dtend;
      ...
      queue->there.u.joint[i] = specptr->end.u.joint[i];
    }
  } else if (queue->type == GO_MOTION_UJOINT) {
    for (i = 0; i < queue->joint_num; i++) {
      neg = specptr->end.u.joint[i] < specptr->start.u.joint[i] ? 1 : 0;
      (void) go_traj_cj_stop(&specptr->cj[i], now, &specptr->cj[i]);
      if (neg) {
  ...
  } else if (queue->type == GO_MOTION_WORLD) {
    ...
    retval = go_motion_spec_interp_world(specptr, endtime, &specptr->end.u.pose);
    if (GO_RESULT_OK != retval) return retval;
    /* else revise the queue's end position */
    queue->there.u.pose = specptr->end.u.pose;
  } else {
...
go_result go_motion_queue_set_id(go_motion_queue * queue, go_integer id)
{
  queue->last_id = id;
...
go_integer go_motion_queue_last_id(const go_motion_queue * queue)
{
  return queue->last_id;
}

go_flag go_motion_queue_is_empty(const go_motion_queue * queue)
{
  return queue->number == 0;
}

go_result go_motion_queue_delete(go_motion_queue * queue)
{
  if (0 == queue->number) {
    return GO_RESULT_OK;
  }
...
go_result go_motion_queue_drop_pending(go_motion_queue * queue)
{
  if (queue->number < 2) {
    return GO_RESULT_OK;
  }
