...
/*
  For C compiles, only declare the wrapper functions at the end.
*/
#ifdef __cplusplus
...
extern int rs274ngc_execute(const char *command = NULL);
...
extern int rs274ngc_restore_tool_table(const char *filename);
...
extern const char *rs274ngc_error(int error_code);
...
/* close the currently open NC code file */
extern int rs274ngc_c_close();

/* execute a line of NC code */
extern int rs274ngc_c_execute(const char *cmd);

/* stop running */
extern int rs274ngc_c_exit();

/* get ready to run */
extern int rs274ngc_c_init();

/* load a tool table */
extern int rs274ngc_c_load_tool_table();

/* open a file of NC code */
extern int rs274ngc_c_open(const char *filename);

/* read the mdi or the next line of the open NC code file */
extern int rs274ngc_c_read();

/* returns the pointer into the error code array */
extern const char *rs274ngc_c_error(int error_code);

/* reset yourself */
extern int rs274ngc_c_reset();

/* restore interpreter variables from a file */
extern int rs274ngc_c_restore_parameters(const char *filename);

/* save interpreter variables to file */
extern int rs274ngc_c_save_parameters(const char *filename, const double parameters[]);

/* synchronize your internal model with the external world */
extern int rs274ngc_c_synch();

/* restore interpreter tool table entries from a file */
extern int rs274ngc_c_restore_tool_table(const char *filename);
