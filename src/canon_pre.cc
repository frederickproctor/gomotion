...
#include "canon.h"
/*
  Change -- the rs274ngc functions were declared 'extern C' so they
  could be linked into a straight C code base. The function here,
  print_nc_line_number(), calls rs274ngc_line_text(), and this was
  declared there explicitly. The explicit declaration was removed and
  replaced with the full include here.
*/
#include "rs274ngc.h"
...
void INIT_CANON()
{
  ...
  for (t = 0; t < sizeof(_tools) / sizeof(*_tools); t++) {
    _tools[t].id = t + 1;
    _tools[t].length = 0.0;
    _tools[t].diameter = 0.0;
  }
}
