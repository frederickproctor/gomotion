/*
  File : rs274ngc_driver.cc
*/

#include <stdio.h>    /* gets, etc. */
#include <stdlib.h>   /* exit       */
#include <string.h>   /* strcpy     */
#include "canon.h"    /* _tools[], _tool_max */
#include "rs274ngc.h"
#include "rs274ngc_return.h"

#define DEFAULT_TOOL_FILE "../etc/rs274ngc.tool"
#define DEFAULT_PARAMETER_FILE "../etc/rs274ngc.var"

// defined in canon_pre.cc
  ...
  if (file_name[0] IS 0) /* ask for name if given name is empty string */
    {
      ...
      buffer[strlen(buffer) - 1] SET_TO 0;
...
int designate_parameter_file(char * file_name)
{
  ...
  fgets(file_name, PARAMETER_FILE_NAME_LENGTH, stdin);
  file_name[strlen(file_name) - 1] SET_TO 0;
...
int main (int argc, char ** argv)
{
  ...
  char default_name[] SET_TO DEFAULT_PARAMETER_FILE;
  int print_stack;
  ...
  if (tool_flag IS 0)
    {
      if (read_tool_file(DEFAULT_TOOL_FILE) ISNT 0)
        exit(1);
    }
